...
import { spendTrendData, topAnomalousDays } from '../mockData';
import { 
...
const CustomTooltip = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-white dark:bg-app-panel border border-slate-200 dark:border-app-border p-3 rounded-lg shadow-xl drop-shadow-[0_0_15px_rgba(0,0,0,0.1)] dark:drop-shadow-[0_0_15px_rgba(0,0,0,0.5)]">
        ...
        <p className="dark:text-white text-slate-900 font-bold flex items-center">
          <span className="w-2 h-2 rounded-full dark:bg-neon-green bg-indigo-500 mr-2 dark:shadow-[0_0_5px_rgba(57,255,20,0.8)]"></span>
...
export default function SpendTrend() {
  ...
  // Identify anomalies for reference dots
    ...
    <div className="max-w-6xl mx-auto animate-fade-in">
      <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="relative">
          <select 
            ...
            className="appearance-none bg-white dark:bg-app-panel border border-slate-300 dark:border-app-border dark:text-white text-slate-700 py-2.5 pl-4 pr-10 rounded-xl shadow-sm text-sm font-medium focus:outline-none focus:ring-1 focus:ring-neon-green focus:border-neon-green cursor-pointer transition-colors"
          >
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={spendTrendData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
              ...
              <Line 
                ...
                strokeWidth={3}
                dot={false}
                activeDot={{ r: 6, fill: '#050505', stroke: '#39ff14', strokeWidth: 3 }}
                style={{ filter: 'drop-shadow(0px 4px 6px rgba(57, 255, 20, 0.4))' }}
              />
      ...
      <div className="glass-panel rounded-2xl overflow-hidden pop-out delay-100">
        <div className="px-6 py-5 border-b dark:border-slate-800 border-slate-200 flex items-center justify-between dark:bg-white/5 bg-slate-50/50">
          ...
          <button className="text-sm font-bold dark:text-neon-green text-indigo-600 hover:text-indigo-700 flex items-center bg-transparent">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr>
                <th className="px-6 py-4 border-b dark:border-slate-800 border-slate-200 dark:bg-app-dark bg-white text-xs font-bold dark:text-slate-400 text-slate-500 uppercase tracking-wider">Date</th>
                <th className="px-6 py-4 border-b dark:border-slate-800 border-slate-200 dark:bg-app-dark bg-white text-xs font-bold dark:text-slate-400 text-slate-500 uppercase tracking-wider">Total Spend</th>
                <th className="px-6 py-4 border-b dark:border-slate-800 border-slate-200 dark:bg-app-dark bg-white text-xs font-bold dark:text-slate-400 text-slate-500 uppercase tracking-wider">Risk Severity</th>
              </tr>
            </thead>
            <tbody className="dark:divide-slate-800 divide-slate-100 divide-y">
              {topAnomalousDays.map((day, idx) => (
                <tr key={idx} className="dark:hover:bg-white/5 hover:bg-slate-50/50 transition-colors">
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium dark:text-white text-slate-900">
                    {day.date}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm dark:text-slate-300 text-slate-600 font-mono">
                    {day.amount}
                  </td>
